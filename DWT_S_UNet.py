...
class Hybrid_Up_4Channel(nn.Module):
    ...
    def __init__(self, in_channels, skip_channels, out_channels, bilinear=True):
        ...
        # 可学习的融合权重（四分支）
        self.weight_dwt_spatial = nn.Parameter(torch.tensor(1.0 / 4))  # SpatialGate(dwt_x)权重
        self.weight_dwt_original = nn.Parameter(torch.tensor(1.0 / 4))  # dwt_x原始权重
        self.weight_unet_spatial = nn.Parameter(torch.tensor(1.0 / 4))  # SpatialGate(unet_x)权重
        self.weight_unet_original = nn.Parameter(torch.tensor(1.0 / 4))  # unet_x原始权重
...
class DWT_S_UNet(nn.Module):
    def __init__(self,
                 ...
                 wave: str = "haar",
                 J: int = 1):
        super(DWT_S_UNet, self).__init__()
...
if __name__ == "__main__":
    # 简单自测：输入 (B, C, L)
    net = DWT_S_UNet(in_channels=1, num_classes=1, bilinear=True, base_c=64, wave="haar", J=1)
