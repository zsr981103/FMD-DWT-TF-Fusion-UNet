...
from  DWT_S_UNet_ReLU import DWT_S_UNet_ReLU
...
def print_dwt_s_unet_skip_weights(
    model_path: str,
    in_channels: int = 4,
    num_classes: int = 1,
    base_c: int = 64,
    ...
    """
    加载 DWT_S_UNet 的 state_dict 并打印四层上采样的时频融合权重。

    参数:
        model_path: 已训练模型的 .pth 文件路径（state_dict 格式）
        in_channels: 输入通道数（训练时是多少就填多少，默认 1）
        num_classes: 输出通道数（默认 1）
        base_c: 基础通道数（默认 64，与 DWT_S_UNet 一致）
        bilinear: 上采样方式（默认 True）
        device: 设备字符串，例如 'cuda' 或 'cpu'，默认自动检测
    """
    if device is None:
    ...
    print("DWT_S_UNet 上采样层 时频融合权重：")
    print("=" * 60)

    for i in range(1, 5):
        up = getattr(net, f"up{i}")

        wd_s = up.weight_dwt_spatial.item()
        wd_o = up.weight_dwt_original.item()
        wu_s = up.weight_unet_spatial.item()
        wu_o = up.weight_unet_original.item()

        print(f"\n----- Up{i} -----")
        print("原始参数值：")
        print(f"  weight_dwt_original  = {wd_o:.6f}")
        print(f"  weight_dwt_spatial   = {wd_s:.6f}")
        print(f"  weight_unet_original = {wu_o:.6f}")
        print(f"  weight_unet_spatial  = {wu_s:.6f}")


        # 绝对值归一化后的占比
        w = torch.tensor([abs(wd_s), abs(wd_o), abs(wu_s), abs(wu_o)], dtype=torch.float32)
        if w.sum() > 0:
            w = w / w.sum()
            print("归一化占比（绝对值）：")
            print(f"  DWT_origin  : {w[1].item() * 100:6.2f}%")
            print(f"  DWT_spatial : {w[0].item() * 100:6.2f}%")
            print(f"  UNet_origin : {w[3].item() * 100:6.2f}%")
            print(f"  UNet_spatial: {w[2].item() * 100:6.2f}%")

        else:
            print("  四个权重绝对值之和为 0，无法归一化。")


# 示例用法
if __name__ == "__main__":
    weights_path = "data/record_result/train_result/LR1e4-rate0.1-40epoch-256_data_size/消融实验/DWTSUNet/model.pth"
    print_dwt_s_unet_skip_weights(weights_path,1)
    ...
    # 示例：打印权重（根据实际模型路径修改）
    # print_dwt_s_unet_skip_weights('data/result/model.pth', in_channels=1)
